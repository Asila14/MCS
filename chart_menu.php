              ...
              <div class="card">
                      ...
                      <div class="form-group row">
                        <div class="col-md-6">
                          ...
                          ?>
                              ...
                              <select data-live-search="true" name="process_id" class="form-control selectpicker" id="select_box">
                                <option>Choose Process</option>
                                <?php
                if ($pro > 0) {
                  while ($pc = sqlsrv_fetch_array($pro, SQLSRV_FETCH_ASSOC)) {
                    // Check if $_POST['process_id'] is set before using it
                    $selected = (isset($_POST['process_id']) && $pc['process_id'] == $_POST['process_id']) ? 'selected' : '';
                    
                    // Check if the process_name is null.
                    if ($pc['process_name'] === null) {
                      // The process_name is null. Display an error message to the user.
                      echo '<p style="align:center; color:red">No records found.</p>';
                    } else {
                      // The process_name is not null. Display the process_name to the user.
                      echo '<option name="process_id" value="' . $pc['process_id'] . '" ' . $selected . '>' . $pc['process_name'] . '</option>';
                    }
                  }
                }
                ?>
               ...
               <!-- Add new process input by user -->
                ...
                if(isset($_POST['search'])) {
                    $query = "SELECT * FROM partnumber INNER JOIN process ON partnumber.process_id=process.process_id WHERE process.process_id = '$process_id' ";
                    $query_run = sqlsrv_query($con, $query);
                    $row_ = sqlsrv_fetch_array($query_run, SQLSRV_FETCH_ASSOC);
              }
                ?>
                <?php
                ...
                if (empty($row_)) {
                    ...
                    if (isset($row_['process_name'])) {
                        // The item name key exists. Display the item name heading.
                        echo '<h4 style="align:center; color:blue;"><b>Process: ' . $row_['process_name'] . '</b></h4><br>';
                    } else {
                      ...
                      <div class="col-md-6">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                ...
                                // Check if the process_id key exists in the $_POST array.
                                if (isset($_POST['process_id'])) {
                                    // The process_id key exists. Get the process ID.
                                    $process_id = $_POST['process_id'];
                                } else {
                                    // The process_id key does not exist. Do not try to access it.
                                    $process_id = '';
                                }
                          ...
                          <div class="col-sm-9">
                            <?php
                                ...
                                <select data-live-search="true" name="measure_id" class="form-control selectpicker">
                                    ...
                                    <?php
                                    if ($lotEnd > 0) {
                                        while ($le = sqlsrv_fetch_array($lotEnd, SQLSRV_FETCH_ASSOC)) {
                                            $selected = (isset($_POST['measure_id']) && $le['measure_id'] == $_POST['measure_id']) ? 'selected' : '';
                                            echo '<option value="' . $le['measure_id'] . '" ' . $selected . '>' . $le['measure_lot'] . '</option>';
                                        }
                                    }
                                    ?>
                                </select>
...
if (isset($_POST['S'])) {
    ...
    $parameters = array($process_id);
    $conditions = array();

        switch (true) {
      /*case !empty($id) && !empty($item_id):
        $conditions[] = "partnumber.id = ? AND item.item_id = ?";
        $parameters[] = $id;
        $parameters[] = $item_id;
        break;*/
      case !empty($id) && !empty($item_id) && !empty($mc_id):
        $conditions[] = "partnumber.id = ? AND item.item_id = ? AND machine.mc_id = ?";
        $parameters[] = $id;
        $parameters[] = $item_id;
        $parameters[] = $mc_id;
        break;
     case !empty($id) && !empty($item_id) && !empty($mc_id) && !empty($a) && !empty($b):
        $conditions[] = "(partnumber.id = ? AND item.item_id = ? AND machine.mc_id = ? AND measure_lot BETWEEN ? AND ?)";
        $parameters[] = $id;
        $parameters[] = $item_id;
        $parameters[] = $mc_id;
        $parameters[] = $a;
        $parameters[] = $b;
        break;
    case !empty($id) && !empty($item_id) && !empty($mc_id) && !empty($a) && !empty($b) && !empty($x) && !empty($y):
        $conditions[] = "(partnumber.id = ? AND item.item_id = ? AND machine.mc_id = ? AND measure_lot BETWEEN ? AND ? AND measure_datetime BETWEEN ? AND ?)";
        $parameters[] = $id;
        $parameters[] = $item_id;
        $parameters[] = $mc_id;
        $parameters[] = $a;
  ...
  // Move the ORDER BY clause inside the WHERE clause
  $query .= " ORDER BY spec_result.result_datetime ASC";
